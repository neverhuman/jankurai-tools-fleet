...
# audit-ci / proof-routing / contract-drift / release-readiness / cost-budget
...
bash ops/ci/governed-jankurai audit . --mode ratchet --baseline target/jankurai/accepted-baseline.json --json target/jankurai/repo-score.json --md target/jankurai/repo-score.md --repair-queue-jsonl target/jankurai/repair-queue.jsonl --full
...
# crate's score-diff projection and property suites exercise the bad-behavior
# guards over the audit substrate; their log is the adoption evidence.
log "tool-adoption: language bad-behavior tests"
cargo test -p jankurai-fleet 2>&1 | tee target/jankurai/language-bad-behavior.log
...
bash ops/ci/governed-jankurai vibe coverage --source agent/vibe-coverage.toml --json target/jankurai/vibe-coverage.json --md target/jankurai/vibe-coverage.md
